...
再打开一个太阳神三国杀窗口，这次点击「加入游戏」，输入你的玩家名称。`主机` 请填写你的 VPS 公网 IP，例如 `114.5.1.4`。如果端口不是默认的 9527，则后面需要跟上你设定的端口号，例如 `114.5.1.4:9528`。为了方便，还是建议配置为默认端口，这样只需输入 IP 即可。点击「连接」，如显示这个界面，即为成功。
